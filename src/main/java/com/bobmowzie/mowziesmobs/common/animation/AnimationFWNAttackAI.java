...
public class AnimationFWNAttackAI<T extends MMEntityBase & IAnimatedEntity> extends AnimationAttackAI<T> {
    ...
    public void updateTask() {
        ...
        if (animatingEntity.getAnimationTick() == (getAnimation().getDuration() / 2)) {
            animatingEntity.playSound(attackSound, 1.2F, 1);
        }

        if (animatingEntity.getAnimationTick() == (getAnimation().getDuration() / 2 + 2)) {
            animatingEntity.playSound("mowziesmobs:wroughtnautSwing1", 1.5F, 1F);
