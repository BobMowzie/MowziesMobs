...
public class AnimationSolarBeam<T extends MMEntityBase & IAnimatedEntity> extends AnimationAI<T> {
    ...
    public void updateTask() {
        super.updateTask();
        float radius1 = 1.7f;
        if (animatingEntity.getAnimationTick() == 4 && !animatingEntity.worldObj.isRemote) {
        ...
        if (animatingEntity.getAnimationTick() >= 22) {
            if (entityTarget != null) {
                animatingEntity.getLookHelper().setLookPosition(entityTarget.posX, entityTarget.posY + entityTarget.height / 2, entityTarget.posZ, 1, 90);
            }
        }
    }
}
