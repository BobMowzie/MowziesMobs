	...
	public static void generateHouse(World world, Random rand, BlockPos pos, EnumFacing dir) {
		...
		if (dir == EnumFacing.SOUTH) {
			...
			if (sideHouseDir <= 2) {
				...
				if (sideHouseFacing == EnumFacing.EAST)
					box = new AxisAlignedBB(pos.add(7, 0, 0).add(-2, 1, -2), pos.add(7, 0, 0).add(2, 9, 2));
			...
			if (sideHouseDir <= 2) {
				...
				if (sideHouseFacing == EnumFacing.NORTH)
					box = new AxisAlignedBB(pos.add(0, 0, -7).add(-2, 1, -2), pos.add(0, 0, -7).add(2, 9, 2));
...
//                    else System.out.println("No space");
	...
	private static int findGenHeightBarakoa(World world, BlockPos pos) {
		for (int y = 70 - pos.getY(); y > 50 - pos.getY(); y--) {
			...
			if (world.getBlockState(pos.add(0, y, 0)).getBlock() == Blocks.LEAVES || world.getBlockState(pos.add(0, y, 0)).getBlock() == Blocks.LEAVES2)
				break;
			return y;
		}
//        System.out.println("Failed to find height");
